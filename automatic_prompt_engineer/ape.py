...
from automatic_prompt_engineer import config, data, evaluate, generate, llm, template
...
def estimate_cost(
    ...
    num_prompts = (
        conf["generation"]["num_prompts_per_subsample"]
    ...
    if eval_method == "bandits":
        ...
        if num_prompts_per_round < 1:
            num_prompts_per_round = int(num_prompts * num_prompts_per_round)
        num_evals = (
            conf["evaluation"]["rounds"]
...
def get_generation_query(
    ...
    for _ in range(num_query):
        ...
        queries.append(
            generate.get_query(prompt_gen_template, demos_template, subsampled_data)
        )

    return queries


def get_evaluation_query(
    eval_template,
    demos_template,
    conf,
    eval_data,
    ...
    for _ in range(num_query):
        ...
        query = eval_query(
            filler_prompt, eval_template, input_, output_, demo_data, demos_template
        )[0]
